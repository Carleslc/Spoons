...
function obj:copyPassword(strong, length, chars)
    ...
    local password = obj:generatePassword(strong, length, chars)
    
    -- Set as concealed to avoid showing it in the clipboard history
    -- https://www.hammerspoon.org/docs/hs.pasteboard.html#writeDataForUTI
    hs.pasteboard.setContents(password)
    hs.pasteboard.writeDataForUTI(nil, "org.nspasteboard.ConcealedType", password, true)

    -- UTI types:
    -- public.utf8-plain-text (normal text)
    -- org.nspasteboard.ConcealedType (ignored as sensitive)
    -- org.nspasteboard.AutoGeneratedType (ignored by clipboard history)

    --inspect(hs.pasteboard.contentTypes())

    obj:notify("Password copied to clipboard!")
    return password
end
